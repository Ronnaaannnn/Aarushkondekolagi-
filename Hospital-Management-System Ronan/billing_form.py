...
class Billing:
    def __init__(self,master):
        ...
        #===============TITLE==========
        self.lblTitle = Label(self.frame,text = "BILLING WINDOW", font="Helvetica 20 bold",bg="white", fg="red")
        ...
        self.lbldid = Label(self.LoginFrame,text="DATE DISCHARGED(YYYY-MM-DD)",font="Helvetica 14 bold",bg="white", fg="red",bd=22)
        ...
        self.button2 = Button(self.LoginFrame, text="UPDATE DISCHARGE DATE",width =25,font="Helvetica 14 bold",bg="blue", fg="white",command = self.UPDATE_DATE)
        self.button2.grid(row=1,column=3)
        
        self.lbltreat= Label(self.LoginFrame,text="TREATMENT",font="Helvetica 14 bold",bg="white", fg="red",bd=22)
        ...
        self.lblcode_t1= Label(self.LoginFrame,text="TREATMENT CODE",font="Helvetica 14 bold",bg="white", fg="red",bd=22)
        ...
        self.lblap = Label(self.LoginFrame,text="TREATMENT COST ₹",font="Helvetica 14 bold",bg="white", fg="red",bd=22)
        ...
        self.lblmed = Label(self.LoginFrame,text="MEDICINE",font="Helvetica 14 bold",bg="white", fg="red",bd=22)
        ...
        self.med_t1= Label(self.LoginFrame,text="MEDICINE QUANTITY",font="Helvetica 14 bold",bg="white", fg="red",bd=22)
        ...
        self.lblapd = Label(self.LoginFrame,text="MEDICINE PRICE ₹",font="Helvetica 14 bold",bg="white", fg="red",bd=22)
        ...
        #===========BUTTONS=============    
        self.button3 = Button(self.LoginFrame2, text="UPDATE DATA",width =15,font="Helvetica 14 bold",bg="blue", fg="white",command= self.UPDATE_DATA)
        self.button3.grid(row=3,column=2)
        
        self.button3 = Button(self.LoginFrame2, text="GENERATE BILL",width =15,font="Helvetica 14 bold",bg="blue", fg="white",command= self.GEN_BILL)
    ...
    def UPDATE_DATE(self):
        ...
        b1 = (self.P_id.get())
        b2 =(self.dd.get())  
        c.execute("UPDATE ROOM SET DATE_DISCHARGED=? where PATIENT_ID=?", (b2, b1,))
        conn.commit()
        conn.close()
    ...
    def UPDATE_DATA(self):
        ...
        b7 = (self.med_q.get())
        b8 = (self.price.get())   
        p = list(c.execute("Select * from TREATMENT where TREATMENT.PATIENT_ID=?", (b1,)))  
        if len(p) != 0:
            tkinter.messagebox.showerror("HOSPITAL DATABSE SYSTEM", "PATIENT ID IS ALREADY REGISTERED")
        else:
            c.execute("INSERT INTO TREATMENT VALUES(?,?,?,?)", (b1, b3, b4, b5,))
            c.execute("INSERT INTO MEDICINE VALUES(?,?,?,?)", (b1, b6, b7, b8,))
            conn.commit()
    ...
    def GEN_BILL(self):
        ...
        c = conn.cursor()
        b1 = (self.P_id.get())
        u=c.execute("Select sum(T_COST+ (M_COST*M_QTY) +(DATE_DISCHARGED-DATE_ADMITTED)*RATE) FROM ROOM NATURAL JOIN TREATMENT natural JOIN MEDICINE where PATIENT_ID=?",(b1,) )
        for ii in u:
            self.pp=Label(self.LoginFrame,text="TOTAL AMOUNT OUTSTANDING",font="Helvetica 14 bold",bg="white", fg="red",bd=22)
            self.pp.grid(row=5,column=0)
            self.uu=Label(self.LoginFrame,font="Helvetica 14 bold",bg="white", fg="red",bd=22,text=ii[0])
